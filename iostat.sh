#!/bin/bash
#
# UserParameter=iostat[*],/etc/zabbix/iostat.sh "none" "$1" "$2"
#
export LC_ALL=""
...
if [ ${METRIC} = "kbrs" ]; then
    cat ${CACHE_FILE} | grep ${DISK} | tail -n 1 | awk '{print $6}'
    exit 0
fi
if [ ${METRIC} = "kbws" ]; then
    cat ${CACHE_FILE} | grep ${DISK} | tail -n 1 | awk '{print $7}'
    exit 0
fi
